      ...
      <div className="titles">
        ...
        <div className="center-of-titles">
          <h3
            ...
            style={{
              borderBottom: selectedType === 'videos' ? '2px solid #000' : '',
            }}
            className="videos"
          >
            {' '}
            Videos {movieVideos.results.length}
          </h3>
          <h3
            onClick={typeSelectHandler}
            style={{
              borderBottom: selectedType === 'backdrops' ? '2px solid #000' : '',
            }}
            className="backdrops"
          >
            {' '}
          ...
          <h3
            onClick={(e) => typeSelectHandler(e)}
            style={{
              borderBottom: selectedType === 'posters' ? '2px solid #000' : '',
            }}
            className="posters"
          >
            {' '}
        ...
        <Link to={`/moviedetail/${selectedMovieId}/${selectedType}`}>
          View All {selectedType.charAt(0).toUpperCase()}
          {selectedType.slice(1)}
        </Link>{' '}
        {/* baş harfi büyütme işlemi*/}
      </div>
      <div className="contents">
        {selectedType === 'videos' && Object.keys(movieVideos).length > 0 && (
          <MovieVideos movieVideos={movieVideos} />
        )}

        {selectedType === 'backdrops' && Object.keys(movieImages).length > 0 && (
          <MovieBackdrops movieImages={movieImages} />
        )}
        {selectedType === 'posters' && Object.keys(movieImages).length > 0 && (
          <MoviePosters movieImages={movieImages} />
        )}
      </div>
    </div>
  );
