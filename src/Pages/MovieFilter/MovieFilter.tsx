...
const MovieFilter = () => {
  ...
  const confirmHandler = (e: React.MouseEvent<HTMLElement, MouseEvent>) => {
    ...
    if (endReleaseDate) path += `&release_date.lte=${endReleaseDate}`;

    if (voteCountValue) path += `&vote_count.gte=${voteCountValue}`;

    if (runtimeValue[0]) path += `&with_runtime.gte=${runtimeValue[0]}`;
